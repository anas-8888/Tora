...
const value = window.location.href.split('/')[window.location.href.split('/').length - 1];

async function loadData() {
      try {
            const response = await fetch(`https://d27d-94-47-134-176.ngrok-free.app/stores-info/${value}`);
            if (!response.ok) {
                  throw new Error(`Network response was not ok (status ${response.status})`);
            }
            const data = await response.json();

            const managerDateOfLogin = new Date(data.logDate).toLocaleString('en-US', options1);
      ...
      addNewProduct.addEventListener('submit', (event) => {
            ...
            fetch('/add-new-product', {
                  ...
                  body: JSON.stringify({
                        ...
                        storeId: window.location.href.split('/')[window.location.href.split('/').length - 1],
                  }),
                  headers: {
