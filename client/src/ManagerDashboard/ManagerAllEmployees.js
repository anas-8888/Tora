...
async function loadData() {
      await fetch('https://d27d-94-47-134-176.ngrok-free.app/manager-emplyees-info')
            .then(response => {
                  if (!response.ok) {
            ...
            .then(data => {
                  let managerDateOfLogin = new Date(data.logDate).toLocaleString('en-US', options1);
                  ...
                  const selectBody1 = document.querySelector('#inputEmployeeShopName1');
                  data.shops.forEach(shop => {
                        const row = document.createElement('option');
                        row.innerHTML = `${shop.name}`;
                        row.value = shop.name;
                        selectBody1.appendChild(row);
                  });
                  const selectBody2 = document.querySelector('#inputEmployeeShopName2');
                  data.shops.forEach(shop => {
                        const row = document.createElement('option');
                        row.innerHTML = `${shop.name}`;
                        row.value = shop.name;
                        selectBody2.appendChild(row);
                  });
      ...
      document.querySelector('#edit-employee-form').addEventListener('submit', (event) => {
            ...
            fetch(`/edit-employee/${employeeId}`, {
                  ...
                  body: JSON.stringify({
                        ...
                        shopName: newemployeeData.get('employeeShopName'),
                  }),
                  headers: {
      ...
      addNewEmplyeeForm.addEventListener('submit', (event) => {
            ...
            fetch('/add-new-employee', {
                  ...
                  body: JSON.stringify({
                        ...
                        shopName: employeeData.get('employeeShopName'),
                  }),
                  headers: {
                        'Content-Type': 'application/json'
                  }
...
function drawStuff(menu) {
      if (menu.value === 'Store Keeper') {
            document.querySelector("#inputEmployeeShopName1").style.display = "none";
            document.querySelector("#inputEmployeeShopName1").value = '';
            document.querySelector("#inputEmployeeShopName2").style.display = "none";
            document.querySelector("#inputEmployeeShopName2").value = '';
      } else {
            document.querySelector("#inputEmployeeShopName1").style.display = "block";
            document.querySelector("#inputEmployeeShopName2").style.display = "block";
      }
}
