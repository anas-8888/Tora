...
function fetchProductsAndInitializeTable() {
      ...
      fetch(`https://d27d-94-47-134-176.ngrok-free.app/all-products/emp`)
            .then(response => response.json())
            .then(data => {
                  storeId = data.store._id;
                  shopCashBox = data.shopCashBox;
...
function createNewTable(rows) {
      ...
      const headers = ['#', 'Name', 'O-Price', 'Quantity', 'Total'];
...
function handleKeyPress(event) {
      if (event.key === 'Enter') {
            ...
            if (product) {
                  ...
                  row.querySelector('.price').value = product.originalPrice;
                  row.querySelector('.quantity').value = 1;
                  row.querySelector('.total').value = (product.originalPrice * 1).toFixed(2);
...
function showProductNotFoundAlert(row) {
      if (confirm("You haven't this product. Do you want to add it?")) {
            window.location.href = `/accountant-employee-dashboard/stores/${storeId}`;
      } else {
            row.remove();
            updateTotalSum();
      }
...
function addNewRow() {
      ...
      newRow.innerHTML = `
        ...
        <td><input type="text" class="price form-control" oninput="updateTotal(this)"></td>
...
function updateTotal(input) {
      ...
      const originalPrice = parseFloat(row.querySelector('.price').value) || 0;
      const quantity = parseFloat(row.querySelector('.quantity').value) || 0;
      row.querySelector('.total').value = (originalPrice * quantity).toFixed(2);
      updateTotalSum();
}
...
function updateTotalSum() {
      ...
      // Check if total sum exceeds the shop cash box
      if (totalSum > shopCashBox) {
            alert("The shop cash box is not enough.")
            // Remove the last row
            if (rows.length > 0) {
...
function processBill(withPrint) {
      ...
      for (let i = 0; i < rows.length; i++) {
            ...
            if (nameInput && nameInput.value) {
                  ...
                  billData.push({ barcode, name, originalPrice, quantity, total });
            }
      }
      ...
      const postUrl = '/add-purchases-bill';
      fetch(postUrl, {
            method: 'POST',
            headers: {
